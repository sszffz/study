#!c:\users\ffz\appdata\local\programs\python\python36\python.exe
...
def main():
    ...
    if downloader.isDone() or args.export:
        ...
        with io.open(downloader.type + '.csv', 'w', encoding='utf-8') as f:
            header_line = text.join(u',', data.headers) + '\n'
            header_line = header_line.replace("exchangeDisplay", "ExchangeDisplay")
            f.write(header_line)
            writer = csv.writer(f)
