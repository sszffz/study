...
class Discriminator(nn.Module):
    def __init__(self, input_shape):
        super().__init__()
        # this pipe converages image into the single number
        self.conv_pipe = nn.Sequential(
            ...
            nn.Conv2d(in_channels=DISCR_FILTERS*8, out_channels=1,
                      kernel_size=(4, 4), stride=(1, 1), padding=(0, 0)),
            nn.Sigmoid()
        )
...
class Generator(nn.Module):
    ...
    def __init__(self, output_shape):
        super().__init__()
        # pipe deconvolves input vector into (3, IMAGE_SIZE, IMAGE_SIZE)
...
def iterate_batches(envs, batch_size=BATCH_SIZE):
    ...
    while True:
        ...
        if len(batch) == batch_size:
            # normalizising input between -1 to 1
...
if __name__ == "__main__":
    ...
    for batch_v in iterate_batches(envs):
        # generate extra fake sample, input i 4D, batch, filters, x, y
        ...
        dis_output_fake_v = net_discr(gen_output_v)
